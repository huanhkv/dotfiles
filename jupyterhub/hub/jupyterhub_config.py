...
class FormDockerSpawner(DockerSpawner):
    def _options_form_default(self):
        ...
        <div class="my-custom-row">
            <div class="my-custom-group-btn-label">RAM</div>
            <div class="my-custom-group-btn">
            {
                "".join([
                    f'''
                    <input class="my-custom-d-none" type="radio" id="{i}g" name="mem_limit" value="{i}G" {"checked" if i==min(list_choose_mem) else ""}>
                    <label class="my-custom-btn my-custom-btn-{"" if i==min(list_choose_mem) else "in"}active" style="width: {100/len(list_choose_mem)}%" for="{i}g">{i}GB</label>
                    '''
                        for i in list_choose_mem
                ])
            }
            </div>
        </div>
        <div class="my-custom-row">
            <div class="my-custom-group-btn-label">CPU</div>
            <div class="my-custom-group-btn">
            {
                "".join([
                    f'''
                    <input class="my-custom-d-none" type="radio" id="{i}cores" name="cpu_limit" value="{i}" {"checked" if i==min(list_choose_cpu) else ""}>
                    <label class="my-custom-btn my-custom-btn-{"" if i==min(list_choose_cpu) else "in"}active" style="width: {100/len(list_choose_cpu)}%" for="{i}cores">{i} cores</label>
                    '''
                        for i in list_choose_cpu
                ])
            }
            </div>
        </div>
        ...
        script_form = """
            <script>
                function get_activated(x) {
                    var same_group_x = x.parentElement.children;
                    var activated_btn = [];
                    for (var i=0; i<same_group_x.length; i++) {
                        if (same_group_x[i].className.includes("my-custom-btn-active")) {
                            activated_btn.push(same_group_x[i]);
                        }
                    }
                    return activated_btn;
                }

                function click_event(x) {
                    var activated_btn = get_activated(x);
                    for (var i=0; i<activated_btn.length; i++) {
                        if (x!=activated_btn[i]) {
                            activated_btn[i].classList.remove('my-custom-btn-active');
                            activated_btn[i].classList.add('my-custom-btn-inactive');
                        }
                    }
                    x.classList.remove('my-custom-btn-inactive');
                    x.classList.add('my-custom-btn-active');
                }

                // Add click event
                const button = document.getElementsByClassName('my-custom-btn');
                for (var i=0; i<button.length; i++) {
                    button[i].setAttribute("onclick", "click_event(this)");
                }
            </script>
        """

        return html_form + style_form + script_form
...
# c.DockerSpawner.volume_driver_options
